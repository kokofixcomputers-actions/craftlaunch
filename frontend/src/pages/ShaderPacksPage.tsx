...
export default function ShaderPacksPage() {
  ...
  const search = useCallback(async (newOffset = 0) => {
    setSearching(true);
    try {
      // Use the dedicated searchShaderPacks API
      const result = await api.searchShaderPacks(query, newOffset);
  ...
  const installPack = async (pack: any) => {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        // Import the shader pack to the current instance
        await api.importShaderPack(currentInstanceId, fileInfo);
        setInstalled(pack.project_id);
        setTimeout(() => setInstalled(null), 3000);
      };
      reader.readAsDataURL(blob);
    } catch (e: any) {
      console.error('Failed to install shader pack:', e);
    } finally {
      setInstalling(null);
    }
  };
    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="px-5 py-4 border-b" style={{ borderColor: 'var(--border)', flexShrink: 0 }}>
        <div className="flex items-center gap-3 mb-3">
          ...
          <h1 style={{ fontSize: '1.2rem', fontWeight: 600 }}>Browse Shader Packs</h1>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-5">
        {results.length === 0 && !searching && (
          <div className="text-center py-8" style={{ color: 'var(--text-3)' }}>
            ...
            <div style={{ fontSize: '0.9rem', marginBottom: 8 }}>No shader packs found</div>
